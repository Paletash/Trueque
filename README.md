...
└── app.js       → Lógica, datos, Firebase y generación de PDFs
```
...
- **`#art-photos`** — input file para seleccionar hasta 4 fotos
...
Accesible tras autenticación con Firebase Auth. Contiene 8 sub-páginas:
...
#### `firebaseConfig`
Credenciales para conectar con el proyecto Firebase `trueque-2gm1`. Inicializa Firestore, Auth y Storage.
...
```js
db, auth, storage        → instancias de Firebase
...
#### `initFirebase()`
Inicializa la app de Firebase con las credenciales. Asigna `db`, `auth` y `storage`. Configura la persistencia de sesión en `SESSION` (se pierde al cerrar el tab). Actualiza el indicador visual de conexión.

#### `setDbStatus(estado, label)`
Actualiza el punto de colores y el texto del indicador "Conectado" en la esquina superior.

#### `subscribeToData(callback)`
Abre un listener en tiempo real sobre la colección `movements` ordenada por timestamp descendente. Cada vez que hay un cambio, actualiza `history`, recalcula `points` y ejecuta el callback. Cancela la suscripción anterior si existía.

#### `subscribeToProducts(callback)`
Igual que `subscribeToData` pero para la colección `products`. Mantiene `allProducts` sincronizado en tiempo real.

#### `recalc()`
Recorre `history` y suma o resta cada `delta` al objeto `points` de cada alumno. Se llama cada vez que llega un cambio de Firebase.
...
2. Comprime y sube cada foto a Firebase Storage en `products/{boleta}/{timestamp}_{i}.jpg`
3. Crea el documento en Firestore en la colección `products` con estado `pending`
...
Lee la categoría final del `<select>` del admin. Si difiere de la sugerida por el alumno, genera un comentario automático explicando el ajuste. Usa un `batch` de Firestore para:
1. Actualizar el producto a `status: 'approved'` con la categoría y puntos finales
2. Crear un movimiento automático en la colección `movements` (marcado con `auto: true`)

Todo ocurre de forma atómica — si falla algo, ningún cambio se guarda.
...
#### `doLogin()`
Para alumnos: valida que la boleta exista en `ROSTER` y llama a `enterStudentView`. Para admins: llama a `auth.signInWithEmailAndPassword`. Si falla, muestra el error apropiado sin revelar si fue el correo o la contraseña.
...
#### `doRegister()`
Valida que haya alumno, grado y signo seleccionados. Calcula el delta (`GRADES[grado] * signo`). Guarda el movimiento en Firestore con nombre, grado, signo, delta, fecha, descripción y timestamp.
...
| `logout()` | Cancela suscripciones, cierra sesión en Firebase Auth, limpia estado y regresa al login |
...
initFirebase();                           // Conecta con Firebase
...
La página pública se actualiza cada vez que hay un cambio en cualquiera de las dos colecciones de Firebase, sin necesidad de recargar.
